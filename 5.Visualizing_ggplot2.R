...
car_plot +　geom_point()
car_plot +　geom_point(alpha = 0.2)   #透過性のalphaを加える　
car_plot +  geom_point(aes(color = class), alpha = 0.4)   #data.setのclassごとに色付け
...
car_plot + geom_point(alpha = 0.4, color = "blue", shape = 2, size = 2)

#5.5.2 Exercise
ggplot(data = mpg) + geom_point(aes(x = displ, y = hwy), color = "blue")
ggplot(data = mpg) + geom_point(aes(x = hwy, y = displ, color = cty))
ggplot(data = mpg) + geom_point(aes(x = hwy, y = displ, shape = cty))
ggplot(data = mpg) + geom_point(aes(x = hwy, y = displ, colour = displ < 5))
...
ggplot(data = mpg, aes(x = displ, y = hwy)) +
  ...
  geom_hline(yintercept=20,colour="red") #特定の値にを引く
